...
function normalizeSetupCommands(value) {
  const raw = Array.isArray(value) ? value : String(value || '').split('\n');
  return raw
...
function normalizeDraft(input, existing, actorEmail) {
  ...
  const setupCommands = normalizeSetupCommands(input?.setupCommands || input?.draft?.setupCommands || existing?.draft?.setupCommands || existing?.setupCommands);
...
function buildSessionTemplateFromVersion(record, version) {
  ...
  return {
    ...
    setupCommands: normalizeSetupCommands(version?.setupCommands),
...
function addRunConfig(files, command, setupCommands) {
  const setup = normalizeSetupCommands(setupCommands);
  if (!command && !setup.length) return files;
...
function buildValidationProject(record, source, mode, options = {}) {
  const selectedMode = String(mode || 'starter').toLowerCase();
  const includeHidden = options.includeHidden === true || selectedMode !== 'starter';
  ...
  const command = options.command
    ? String(options.command).trim().slice(0, 1000)
    : selectedMode === 'tests'
      ? source?.testCommand
      : source?.starterCommand;

  if (selectedMode === 'command' && !command) {
    const error = new Error('Enter a command to run');
    error.statusCode = 400;
    throw error;
  }

  const finalFiles = addRunConfig(files, command, source?.setupCommands);
...
function versionNeedsPrewarm(version) {
  const setupCommands = normalizeSetupCommands(version?.setupCommands);
  if (setupCommands.length) return true;
  ...
  return files.some(file => [
    ...
    'requirements.txt',
    'pyproject.toml'
  ].includes(normalizePath(file?.path)));
}

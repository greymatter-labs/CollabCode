...
async function ensureSandbox(sessionId, customConfig) {
  ...
  return SandboxInstance.createIfNotExists({
    ...
    image: String(customConfig.image || process.env.BL_SANDBOX_IMAGE || DEFAULT_IMAGE),
...
async function runProject(options) {
  ...
  const sandbox = await ensureSandbox(options.sessionId, customConfig);

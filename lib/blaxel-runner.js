...
const SANDBOX_NAME_PREFIX = 'collabcode';
...
function normalizeSandboxSegment(value) {
  ...
  const maxLength = MAX_SANDBOX_NAME_LENGTH - SANDBOX_NAME_PREFIX.length - 1;
  ...
  if (!slug) return `run-${hash}`;
  if (slug.length <= maxLength) return slug;

  const suffix = `-${hash}`;
  return `${slug.slice(0, maxLength - suffix.length).replace(/-+$/g, '')}${suffix}`;
}

function normalizeSessionId(sessionId) {
  return normalizeSandboxSegment(sessionId);
}

function getSandboxName(sessionId) {
  const normalized = normalizeSessionId(sessionId);
  if (!normalized) throw new Error('A sessionId is required for Blaxel execution');
  const name = `${SANDBOX_NAME_PREFIX}-${normalized}`.replace(/-+$/g, '');
  if (!/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])$/.test(name)) {
    throw new Error('Could not build a valid Blaxel sandbox name');
  }
  return name;
}
